...
def extract_dump_data(dir_path, ray, name):
    raylist = [ray]
    raylist.append(ray + 1)
    raylist.append(ray - 1)
    onlydirectories = [f for f in listdir(dir_path) if isdir(join(dir_path, f))]
    for subdir1 in onlydirectories:
        if "actual" not in subdir1:
            another_onlydirectories = [f for f in listdir(join(dir_path, subdir1)) if isdir(join(dir_path, subdir1, f))]
            for subdir in another_onlydirectories:
                for i in range(5):
